...
contract MilkSupplyChain {
    ...
    // Hàm cập nhật thông tin của một lô sữa
    function updateMilkBatch(
        ...
        milkBatches[_batchId] = MilkBatch({
            ...
            farmOwner: _farmOwner,
            approvedBy: _approvedBy,
            status: milkBatches[_batchId].status
        });
    }
    ...
    function createMilkBox(
        ...
        require(milkBatches[_batchId].status == BatchStatus.ApprovedByNutrition, "Batch chua duoc phe duyet");
        require(bytes(milkBoxes[_serialNumber].serialNumber).length == 0, "MilkBox da ton tai");
        
        MilkBox storage box = milkBoxes[_serialNumber];
        box.serialNumber = _serialNumber;
        box.batchId = _batchId;
        box.boxName = _boxName;
        box.manufacturingDate = _manufacturingDate;
        box.expirationDate = _expirationDate;
        box.manufacturer = _manufacturer;
        box.distributorAddress = _distributorAddress;
        box.owner = participants[msg.sender].name;
        box.status = MilkBoxStatus.Created;
        
        emit MilkBoxCreated(_serialNumber, _batchId);
    }
    
    // Cập nhật thông tin hộp sữa
    function updateMilkBox(
        string memory _serialNumber,
        string memory _boxName,
        string memory _manufacturingDate,
        string memory _expirationDate,
